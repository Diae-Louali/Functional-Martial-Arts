...
if (empty($_POST['Article_Id'])) {
    $_POST['Article_Id']= 29; // HARD CODED FOR NOW BUT NEEDS TO BECOME DYNAMIC AND SEND TO THE MOST RECENT OR TRENDING ARTICLE
};

$query = "SELECT * FROM comments_and_replies AS c NATURAL JOIN user AS u
...
{
 $query = "SELECT * FROM comments_and_replies AS c NATURAL JOIN user AS u WHERE c.parent_comment_ID = ? AND u.Id = ?";
 $output = '';
 $statement = $conn->prepare($query);
 $statement->execute(array($parent_id, '30'));
 $result = $statement->fetchAll();
